...
public class DateSortedExpandableListAdapter extends BaseExpandableListAdapter {
    ...
    boolean mDataValid;
    private GroupNameChangeListener mListener;
    ...
    /**
     * Get the byte array at cursorIndex from the Cursor. Assumes the Cursor has
     * already been moved to the correct position. Along with {@link #getInt}
     * and {@link #getString}, these are provided so the client does not need to
     * access the Cursor directly
     * 
     ...
     * already been moved to the correct position. Along with {@link #getBlob}
     * and {@link #getString}, these are provided so the client does not need to
     * access the Cursor directly
     * 
     * @param cursorIndex Index to query the Cursor.
     ...
     * already been moved to the correct position. Along with {@link #getInt}
     * and {@link #getInt}, these are provided so the client does not need to
     * access the Cursor directly
     * 
     * @param cursorIndex Index to query the Cursor.
    ...
    public String getGroupName(int groupPosition) {
        if (groupPositionToBin(groupPosition) == -1) {
            return mDateSorter.getLabel(0);
        }
     ...
     * @return int Group position of the containing group. /* package
     */
    ...
    protected int groupPositionToBin(int groupPosition) {
        if (!mDataValid)
            return -1;
        // for groupPosition max num is 6 and day count is 5.
        if (groupPosition < 0 || groupPosition >= DateSorter.DAY_COUNT + 1) {
        ...
        while (groupPosition > -1) {
            arrayPosition++;
            if (arrayPosition >= mItemMap.length) {
                return -1;
            }
    ...
    public void setGroupNameChangeListener(GroupNameChangeListener l) {
        mListener = l;
    }
    ...
    public View getGroupView(int groupPosition, boolean isExpanded,
        ...
        if (null == convertView || !(convertView instanceof TextView)) {
            ...
            convertView = factory.inflate(R.layout.history_header, null);
            item = (TextView) convertView.findViewById(R.id.group_name);
        } else {
            item = (TextView) convertView.findViewById(R.id.group_name);
        }
        String label = mDateSorter.getLabel(groupPositionToBin(groupPosition));
        if (mListener != null && groupPosition == 0) {
            mListener.changeGroupName(label);
        }
        item.setText(label);
        return item;
    }
