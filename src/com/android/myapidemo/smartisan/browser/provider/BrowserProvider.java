...
public class BrowserProvider extends ContentProvider {
    ...
    // 14 -> 15 move couple of tables to either browser private database or
    // webview database
    ...
    // Regular expression which matches http://, followed by some stuff,
    // followed by
    ...
    public static String getClientId(ContentResolver cr) {
        ...
        // search_client_id includes search prefix, legacy client_id does not
        // include prefix
        try {
            searchClientIdCursor = cr.query(Uri.parse("content://com.google.settings/partner"),
                    new String[] {
                        "value"
                    }, "name='search_client_id'", null, null);
...
//                    CharSequence bookmarkDestination = replaceSystemPropertyInString(mContext,
//                            bookmarks[i + 1]);
...
//            }
     ...
     * "Search the web". Here are the rules. 1. We only have
     * MAX_SUGGESTION_LONG_ENTRIES in the list plus "Search the web"; 2. If
     * bookmark/history entries has a match, "Search the web" shows up at the
     * second place. Otherwise, "Search the web" shows up at the first place.
     */
    private class MySuggestionCursor extends AbstractCursor {
        ...
        public MySuggestionCursor(Cursor hc, Cursor sc, String string) {
            ...
            // Some web suggest providers only give suggestions and have no
            // description string for
            // items. The order of the result columns may be different as well.
            // So retrieve the
            // column indices for the fields we need now and check before using
            // below.
            ...
            if ((mPos != -1 && mHistoryCursor != null)) {
                ...
                switch (columnIndex) {
                    ...
                    case SUGGEST_COLUMN_ICON_1_ID:
                        // if (type == 1) {
                        /*
                         * if (mHistoryCursor.getInt(3) == 1) { return
                         * Integer.valueOf(
                         * R.drawable.ic_search_category_bookmark) .toString();
                         * } else { return Integer.valueOf(
                         * R.drawable.ic_search_category_history) .toString(); }
                         */
                        // } else {
                        /*
                         * return Integer.valueOf(
                         * R.drawable.ic_search_category_suggest) .toString();
                         */
                        // }
                    ...
                    case SUGGEST_COLUMN_QUERY_ID:
                        if (type == 0) {
                            ...
                            // Return the url in the intent query column. This
                            // is ignored
                            // within the browser because our searchable is set
                            // to
                            // android:searchMode="queryRewriteFromData", but it
                            // is used by
                            // global search for query rewriting.
                            return mHistoryCursor.getString(1);
                        } else {
                            if (mSuggestQueryId == -1)
                                return null;
                    ...
                    case SUGGEST_COLUMN_INTENT_EXTRA_DATA:
                        if (type == 0) {
                            ...
                            if (mSuggestIntentExtraDataId == -1)
                                return null;
        ...
        /**
         * Provides the title (text line 1) for a browser suggestion, which
         * should be the webpage title. If the webpage title is empty, returns
         * the stripped url instead.
         * 
        ...
        /**
         * Provides the subtitle (text line 2) for a browser suggestion, which
         * should be the webpage url. If the webpage title is empty, then the
         * url should go in the title instead, and the subtitle should be empty,
    ...
    public Cursor query(Uri url, String[] projectionIn, String selection,
        ...
        Cursor c = mOpenHelper.getReadableDatabase().query(TABLE_NAMES[match % 10], projection,
                /*DatabaseUtils.concatenateWhere(whereClause, selection)*/null, selectionArgs,
...
//            }
    ...
    public int update(Uri url, ContentValues values, String where,
        ...
        if (match == URI_MATCH_BOOKMARKS_ID || match == URI_MATCH_BOOKMARKS) {
            ...
            // if this *is* a bookmark row we're altering, we need to back it
            // up.
    ...
    /**
     * Strips the provided url of preceding "http://" and any trailing "/". Does
     * not strip "https://". If the provided string cannot be stripped, the
     * original string is returned. TODO: Put this in TextUtils to be used by
     * other packages doing something similar.
     * 
     * @param url a url to strip, like "http://www.google.com/"
     * @return a stripped url like "www.google.com", or the original string if
     *         it could not be stripped
     */
