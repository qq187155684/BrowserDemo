 ...
 */
package com.android.myapidemo.smartisan.search;
...
public class OpenSearchSearchEngine implements SearchEngine {
    ...
    public Cursor getSuggestions(Context context, String query) {
        ...
        try {
             ...
             */
            if(mSearchEngineInfo != null && TextUtils.equals("baidu",mSearchEngineInfo.getName())){
                // baidu is a little special
                if(content.startsWith("window.baidu.sug")){
                    content = content.substring("window.baidu.sug".length());
                    try {
                        JSONObject results = new JSONObject(content);
                        JSONArray suggestions = results.getJSONArray("s");
                        JSONArray descriptions = null ;
                        if (results.has("description")) { // this is not real , just assume it has it , when we get
                            // the correct parameter names , correct this
                            descriptions = results.getJSONArray("description");
                        }
