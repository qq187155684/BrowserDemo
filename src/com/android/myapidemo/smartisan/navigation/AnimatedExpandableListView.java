...
public class AnimatedExpandableListView extends ExpandableListView {
    ...
    public static abstract class AnimatedExpandableListAdapter extends BaseExpandableListAdapter {
        ...
        protected boolean isDoAnimation(int groupPosition) {
            return isAnimation;
        }
        ...
        public final View getChildView(final int groupPosition, int childPosition, boolean isLastChild, View convertView, final ViewGroup parent) {
            ...
            if (info.animating) {
                ...
                for (int i = info.firstChildPosition; i < len; i++) {
                    ...
                    totalHeight += childView.getMeasuredHeight() + listView.getDividerHeight();

                    if (totalHeight < clipHeight) {
                        // we only need to draw enough views to fool the user...
                        dummyView.addFakeView(childView);
                        visiableHeight += childView.getMeasuredHeight() + listView.getDividerHeight();
                    } else {
                        dummyView.addFakeView(childView);
                        visiableHeight += childView.getMeasuredHeight() + listView.getDividerHeight();
                ...
                if (info.expanding && state != STATE_EXPANDING) {
                    ExpandAnimation ani = new ExpandAnimation(dummyView, 0, visiableHeight, info);
                    ani.setDuration(this.parent.getAnimationDuration());
                    ani.setInterpolator(new DecelerateInterpolator(1.0f));
                    ...
                    ValueAnimator valueAnimator = ValueAnimator.ofObject(new TypeEvaluator<Number>() {
                        @Override
                        public Number evaluate(float fraction, Number startValue, Number endValue) {
                            float startFloat = startValue.intValue();
                            float retVal = startFloat + fraction * (endValue.intValue() - startFloat);
                            dummyView.setMaskColorAlpha(retVal);
                            return null;
                        }
                    },1, 0);
                    valueAnimator.setInterpolator(new DecelerateInterpolator(1.0f));
                    valueAnimator.setDuration(this.parent.getAnimationDuration());
                    valueAnimator.start();
                    ...
                    if (info.dummyHeight == -1) {
                        info.dummyHeight = visiableHeight;
                    }
    ...
    private static class DummyView extends View {
        ...
        public DummyView(Context context) {
            super(context);
            maskColor = Color.argb((int) (255 * 0.3), 0, 0, 0);
        }

        public void setMaskColorAlpha(float val) {
            maskColor = Color.argb((int) (255 * 0.3 * val), 0, 0, 0);
        }
