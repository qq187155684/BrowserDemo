...
public class TabControl {
    ...
    private static final String POSITIONS_INCOG = "positions_incog";
    private static final String CURRENT_CID = "current_cid";
    private static final String CURRENT_INCOG_CID = "current_incog_cid";
    ...
    // Private array of WebViews that are used as tabs.
    private ArrayList<Tab> mIncogTabs;
    // Queue of most recently viewed tabs.
    private ArrayList<Tab> mTabQueue;
    // Current position in mTabs.
    private int mCurrentPosition = -1;
    // Current incog position in mIncogTabs.
    private int mCurrentIncogPosition = -1;
    // the main browser controller
    private final Controller mController;
    private boolean mCurrentAllTabsMode;
    ...
    TabControl(Controller controller) {
        ...
        mIncogTabs = new ArrayList<Tab>(mMaxTabs);
        mTabQueue = new ArrayList<Tab>(2*mMaxTabs);
    }
    ...
    public ArrayList<Tab> getList(boolean isPrivate) {
        if (isPrivate) {
            return mIncogTabs;
        } else {
            return mTabs;
    ...
    public ArrayList<Tab> getAllList() {
        return mTabQueue;
    }
    /**
    ...
    Tab getTab(int position) {
        System.out.println("====================== TabControl position ========================"+position);
        if (mCurrentAllTabsMode) {
            if (position >= 0 && position < mIncogTabs.size()) {
                return mIncogTabs.get(position);
            }
        } else {
    ...
    public void setIncogMode(boolean isIncogState) {
        mCurrentAllTabsMode = isIncogState;
        changeBundleMode();
    }

    private void changeBundleMode() {
        mController.backupState();
    }

    public boolean getIncogMode() {
        return mCurrentAllTabsMode;
    }
    ...
    public Tab getCurrentTab() {
        System.out.println("=================== getCurrentTab mCurrentIncogPosition ===================="+mCurrentIncogPosition);
        Thread.dumpStack();
        System.out.println("=================== getCurrentTab mCurrentPosition ===================="+mCurrentPosition);
        if (mCurrentAllTabsMode) {
            return getTab(mCurrentIncogPosition);
        } else {
            return getTab(mCurrentPosition);
        }
    }
    ...
    public boolean canCreateNewTab() {
        if (mCurrentAllTabsMode) {
            return mMaxTabs > mIncogTabs.size();
        } else {
    ...
    /**
     * Create a new tab or restore current tab.
     * @return if restore a private tab, then return this tab,
     * and set first tab of normal mode as current tab.
     * else create and return a new tab.
     */
    Tab createNewTab(Bundle state, boolean privateBrowsing, boolean background) {
        // Return false if we have maxed out on tabs
        if ((privateBrowsing && mIncogTabs.size() >= mMaxTabs) || (!privateBrowsing && mTabs.size() >= mMaxTabs)) {
            return null;
        }
        final WebView w = createNewWebView(privateBrowsing, background);
        // Create a new tab and add it to the tab list
        Tab t = new Tab(mController, w, state);
        if (state != null) {
            // The following code use restore private tabs before, but now for
            // not need to restore private tabs so not do the code.
            if (t.isPrivateBrowsingEnabled()) {
                mIncogTabs.add(t);
                mCurrentIncogPosition = mIncogTabs.indexOf(t);
                try {
                    t = mTabs.get(mCurrentPosition);
                } catch (Exception e) {
                    return null;
                }
            } else {
    ...
    boolean removeTab(Tab t) {
        ...
        // save backforward list to db
        if (!t.isDownoadUrl()) {
            DataController.getInstance(mController.getActivity()).saveState(t.saveBackForwardList());
        }
        ...
        if (current == t) {
            ...
            if (mCurrentAllTabsMode) {
                if (getTabPosition(current) != -1) {
                    mCurrentIncogPosition = getTabPosition(current);
                }
            } else {
                if(getTabPosition(current) != -1){
                    mCurrentPosition = getTabPosition(current);
                }
            }
        }
        // destroy the tab
    ...
    void saveState(Bundle outState) {
        saveTabsKey(outState, false);
        saveTabsKey(outState, true);
    }

    private void saveTabsKey(Bundle outState, boolean isIncog) {
        final int numTabs;
        if (isIncog) {
            numTabs = mIncogTabs.size();
        } else {
            numTabs = mTabs.size();
        }
        if (numTabs == 0) {
        ...
        for (Tab tab : tempTabs) {
            if (tab.isShowHomePage() || tab.isDownoadUrl()) {
                continue;
            }
            ...
            if (!outState.isEmpty()) {
                ...
                if (isIncog) {
                    try {
                        current = mIncogTabs.get(mCurrentIncogPosition);
                    } catch (Exception e) {
                        mCurrentIncogPosition = 0;
                        current = mIncogTabs.get(mCurrentIncogPosition);
                    }
                    if (current != null) {
                        cid = current.getId();
                        outState.putLong(CURRENT_INCOG_CID, cid);
                        outState.putLongArray(POSITIONS_INCOG, ids);
                    }
                } else {
                    try {
                        current = mTabs.get(mCurrentPosition);
                    } catch (Exception e) {
                        mCurrentPosition = 0;
                        current = mTabs.get(mCurrentPosition);
                    }
                    if (current != null) {
                        cid = current.getId();
                        outState.putLong(CURRENT_CID, cid);
     ...
     */
    long canRestoreState(Bundle inState, boolean incog, boolean restoreIncognitoTabs) {
        final long[] ids;
        final long oldcurrent;
        if (incog) {
            ids = (inState == null) ? null : inState.getLongArray(POSITIONS_INCOG);
            if (ids == null) {
                return -1;
            }
            oldcurrent = inState.getLong(CURRENT_INCOG_CID);
        } else {
            ids = (inState == null) ? null : inState.getLongArray(POSITIONS);
            if (ids == null) {
                return -1;
            }
            oldcurrent = inState.getLong(CURRENT_CID);
        }
        long current = -1;
     ...
     */
    void restoreState(Bundle inState, long currentId, long currentIncogId,
            boolean restoreIncognitoTabs, boolean restoreAll) {
        if (currentId == -1) {
            return;
        }
        long[] ids = inState.getLongArray(POSITIONS);
        long[] idsIncog = inState.getLongArray(POSITIONS_INCOG);
        HashMap<Long, Tab> tabMap = new HashMap<Long, Tab>();
        restoreTabs(ids, tabMap, inState, currentId, restoreIncognitoTabs, restoreAll);
        restoreTabs(idsIncog, tabMap, inState, currentIncogId, restoreIncognitoTabs, restoreAll);
    ...
    private void restoreTabs(long[] ids,HashMap<Long, Tab> tabMap,Bundle inState,
            long currentId,boolean restoreIncognitoTabs, boolean restoreAll){
        if (ids == null){
            return;
        }
        for (long id : ids) {
            ...
            if (state == null || state.isEmpty()) {
                ...
                // default restore normal tab
                Tab t = createNewTab(state, state.getBoolean(Tab.INCOGNITO));
                if (t == null) {
                    // We could "break" at this point, but we want
                    // sNextId to be set correctly.
                    continue;
                }
                DataController.getInstance(mController.getContext()).clearState(state.getString("currentUrl")); // delete from db after restore
                tabMap.put(id, t);
    ...
    private Vector<Tab> getHalfLeastUsedTabs(Tab current) {
        ...
        // null.
        if ((mTabs.size() == 1 && mIncogTabs.size() <= 1) || current == null) {
            return tabsToGo;
        }
        ...
        for (Tab t : mTabQueue) {
            if (t != null && t.getWebView() != null) {
                openTabCount++;
                if (t.isPrivateBrowsingEnabled() && mIncogTabs.size() == 1) {
                    continue;
                }
                if (!t.isPrivateBrowsingEnabled() && mTabs.size() == 1) {
                    continue;
                }
    ...
    Tab getLeastUsedTab(Tab current) {
        ...
        for (Tab t : mTabQueue) {
            if (t != null) {
                if (t != current && t != current.getParent()) {
                    if (t.isPrivateBrowsingEnabled() != mCurrentAllTabsMode) {
                        continue;
                    } else {
                        return t;
                    }
     ...
     */
    public Tab findTabWithUrl(String url, boolean incognito) {
        if (url == null) {
            return null;
        }
        // Check the current tab first.
        Tab currentTab = getCurrentTab();
        if (currentTab != null && currentTab.getWebView() != null && tabMatchesUrl(currentTab, url) && currentTab.isPrivateBrowsingEnabled() == incognito) {
            return currentTab;
        }
        // Now check all the rest.
        for (Tab tab : getList()) {
            if ((tab.getWebView() != null && tabMatchesUrl(tab, url) && tab.isPrivateBrowsingEnabled() == incognito) || (TextUtils.isEmpty(url) && tab.isShowHomePage())) {
                return tab;
            }
        }
        return null;
    ...
    private WebView createNewWebView(boolean privateBrowsing, boolean background) {
        //return mController.getWebViewFactory().createWebView(privateBrowsing, background);
    ...
    private boolean setCurrentTab(Tab newTab, boolean force) {
        ...
        if (mCurrentAllTabsMode) {
            mCurrentIncogPosition = mIncogTabs.indexOf(newTab);
        } else {
            mCurrentPosition = mTabs.indexOf(newTab);
        }
