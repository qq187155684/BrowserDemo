...
public class IntentHandler {
    ...
    void onNewIntent(Intent intent) {
        ...
        if (Intent.ACTION_VIEW.equals(action)
            ...
            if (intent.getBooleanExtra(Browser.EXTRA_CREATE_NEW_TAB, false)
                  || urlData.isPreloaded()) {
                //if create a new tab, then do dismissReadWindow with animator.
                if (mController.getUi().isReadModeWindowShowing())
                    mController.getUi().dismissReadModeWindow();
                // if current tab is empty,reuse it
                if (TextUtils.isEmpty(current.getUrl())) {//0.if current tab is empty
                    mController.loadUrlDataIn(current, urlData);
                } else {
                    if (mTabControl.canCreateNewTab()) {//1.if can create tab, just create it
                        mController.openTab(urlData);
                    } else {
                        Tab tab = mTabControl.findTabWithUrl("", false);
                        if (tab != null) {//2.if can't create but has empty tab,reuse it
                            tab.setShowHomePage(false);
                            mController.setActiveTab(tab);
                            mController.loadUrlDataIn(tab, urlData);
                        }else{//3.no empty tab,we just close the leastused tab and create a new tab.
                            mController.getUi().closeTheLeastUsedTab();
                            mController.openTab(urlData);
                        }
                    }
            ...
            //if read window showing, and jump to a new url, then do dismissReadWindow without animator.
            if (mController.getUi().isReadModeWindowShowing())
                mController.getUi().cancelshowReadModeWindow();
            /*
            ...
            if (Intent.ACTION_VIEW.equals(action)
                ...
                if (!BrowserActivity.isTablet(mActivity)
                    ...
                    Tab appTab = mTabControl.getTabFromAppId(appId);
                    if (appTab != null) {
                        mController.changeIncogModeStyle(mTabControl.getIncogMode());
                        appTab.setShowHomePage(false);
                ...
                // with a matching url.
                Tab appTab = mTabControl.findTabWithUrl(urlData.mUrl, false);
                if (appTab != null) {
                    // Transfer ownership
                    ...
                    // exit on close.
                    if (TextUtils.isEmpty(current.getUrl())) {//0.if current tab is empty
                        mController.getUi().hideHomePage();
                        mController.loadUrlDataIn(current, urlData);
                        ...
                        } else {//2.if can't create but has empty tab,reuse it
                            Tab tab = mTabControl.findTabWithUrl("", false);
                            if (tab != null) {
                                tab.setAppId(appId);
                                tab.setShowHomePage(false);
                                ...
                                mController.getUi().closeTheLeastUsedTab();
                                tab = mController.openTab(urlData);
                                if (tab != null) {
                                    tab.setAppId(appId);
                                    if ((intent.getFlags() & Intent.FLAG_ACTIVITY_BROUGHT_TO_FRONT) != 0) {
                                        tab.setCloseOnBack(true);
                    ...
                    reloadIfNetworkNotAvailable(mActivity); // for 41683, to avoid cache influence
                }
            } else {
    ...
    private void reloadIfNetworkNotAvailable(Context context) {
        ...
        if (activeNetworkInfo == null || !activeNetworkInfo.isAvailable()) {
            new Handler().postDelayed(new Runnable() {
                @Override
                public void run() {
                    mTabControl.getCurrentTab().getWebView().reload();
                }
            }, 300);
        }
    }
    ...
    static boolean handleWebSearchIntent(Activity activity,
        ...
        if (intent.getBooleanExtra(Browser.EXTRA_CREATE_NEW_TAB, false) && controller != null
                && controller.getTabControl().canCreateNewTab()) {
            return false;
        }
    ...
    private static boolean handleWebSearchRequest(Activity activity,
        ...
        if (BrowserSettings.getInstance() == null) {
            BrowserSettings.initialize(activity);
        }
