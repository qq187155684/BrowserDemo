...
import com.android.myapidemo.smartisan.browser.provider.BrowserProvider2.OmniboxSuggestions;
...
public class SuggestionsAdapter extends BaseAdapter implements Filterable,
    ...
    private Object mQueryLock = new String("query_lock");
    ...
    private SlowFilterTask mRunningTask;
    private Handler mHandler ;
    private int mType = UrlInputView.ADDRESS_INPUT;
    ...
    public SuggestionsAdapter(Context ctx, CompletionListener listener) {
        ...
        HandlerThread thread = new HandlerThread("suggestion-query");
        thread.start();
        mHandler = new Handler(thread.getLooper()){
            ...
            public void handleMessage(Message msg){
                ...
                SlowFilterTask task = new SlowFilterTask();
                mRunningTask = task;
                task.execute((CharSequence)msg.obj);
            }
        };
    }
    ...
    public void setType(int type) {
        mType = type;
        if (mType == UrlInputView.ADDRESS_INPUT)
    ...
    private void addSource(CursorSource c) {
        if (mSources == null) {
    ...
    class SuggestFilter extends Filter {
        ...
        void startSuggestionsAsync(final CharSequence constraint) {
            if (!mIncognitoMode && mHandler != null) {
                mHandler.removeMessages(0);
                mHandler.sendMessageDelayed(mHandler.obtainMessage(0, constraint), 100L);
            }
        }
        ...
        protected FilterResults performFiltering(CharSequence constraint) {
            ...
            if (constraint != null && mSources != null) {
        ...
        void mixResults(List<SuggestItem> results) {
            HashMap<String, SuggestItem> bookmarkMap = new HashMap<String, SuggestItem>();
            HashMap<String, SuggestItem> historyMap = new HashMap<String, SuggestItem>();
            for (int i = 0; i < mSources.size(); i++) {
                CursorSource s = mSources.get(i);
                for (int j = 0; j < s.getCount(); j++) {
                    SuggestItem item = s.getItem();
                    if (item.type == TYPE_BOOKMARK)
                        bookmarkMap.put(item.url, item);
                    else
                        historyMap.put(item.url, item);
                    s.moveToNext();
                }
            }
            ...
            int count = 0;
            for (String url : bookmarkMap.keySet()) {// bookmark prefered
                if (count >= maxLines) break;
                results.add(bookmarkMap.get(url));
                count++;
            }
            for (String url : historyMap.keySet()) {
                if (count >= maxLines) break;
                if (!bookmarkMap.containsKey(url)) {
                    results.add(historyMap.get(url));
                    count++;
                }
            }
        }
    ...
    private int getMaxLines() {
        ...
        maxLines = (int) Math.ceil(maxLines);
        return maxLines;
    }
    ...
    class SuggestionResults {
        ...
        void addResult(SuggestItem item) {
            ...
            while ((ix < items.size()) && (item.type >= items.get(ix).type)) {
                if (item.url != null) {
                    if (item.url.equals(items.get(ix).url)) return; // not insert duped url
                }
                else {
                    if (item.title .equals(items.get(ix).url)) return; // not insert duped url
                }
                ix++;
            }
            items.add(ix, item);
