...
public class UrlHandler {
    ...
    boolean shouldOverrideUrlLoading(Tab tab, WebView view, String url) {
        if (url.length() > 512000) {
            // if url.length is longer than 512000(not accurate), PackageManager.resolveActivity() will make package manager crash and browser get RuntimeException
            return false;
        }
    ...
    boolean startActivityForUrl(Tab tab, String url) {
      ...
      intent.addCategory(Intent.CATEGORY_BROWSABLE);
      //TODO:By default using our browser application.
//      intent.setPackage("com.android.browser");
      ...
      if (!url.toLowerCase().startsWith("mailto:")) {// workaround for can't launch email
    ...
    private void closeTabWithDelay(final Tab tab) {
        if (tab.getLoadProgress() == 0) {
