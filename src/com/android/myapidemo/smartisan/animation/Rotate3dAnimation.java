...
public class Rotate3dAnimation extends Animation {
    ...
    private final boolean mIncogState;
    ...
    public Rotate3dAnimation(float fromDegrees, float toDegrees, float centerX, float centerY,
            float depthZ, boolean reverse, boolean isToIncog) {
    ...
    public interface onAnimationLinstener {
        public void updateValues(float x);
    }
    ...
    protected void applyTransformation(float interpolatedTime, Transformation t) {
        ...
        camera.save();
        camera.setLocation(camera.getLocationX(), camera.getLocationY(), -30f);
        if (mReverse) {
        ...
        if (mIncogState) {
            if (degrees >= 270) {
                animScale = 1 - 0.2f * interpolatedTime;
            } else {
                animScale = 0.8f + 0.2f * interpolatedTime;
            }
        } else {
            if (degrees <= 90) {
                animScale = 1 - 0.2f * interpolatedTime;
